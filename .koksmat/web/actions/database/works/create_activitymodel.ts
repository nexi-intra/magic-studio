...
// The schema for the CreateActivitymodel procedure
const _schema = z
  .object({
...
export type CreateActivitymodelProps = z.infer<typeof _schema>;
...
/**
// Example usage of the  function CreateActivitymodel
try {
  const  result =  CreateActivitymodel(
...
export default async function CreateActivitymodel(
  authtoken: string,
  name: string,
  description: string,
  activity: string,
  ...
  const input = { name, description, activity, data };
  ...
  const result = await execute(
    ...
    "create_activitymodel", // <-- this is the name of the procedure in the database pointed to by the connection
