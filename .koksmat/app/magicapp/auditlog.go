...
func (auditLog *PowerShellLog) Collection() *mgm.Collection {
	...
	client, err := mgm.NewClient(options.Client().ApplyURI(utils.MongoConnectionString()))
	if err != nil {
		panic(err)
	}
	// Get the model's db
...
func (auditLog *AuditLogSum) Collection() *mgm.Collection {
	...
	client, err := mgm.NewClient(options.Client().ApplyURI(utils.MongoConnectionString()).SetMonitor(cmdMonitor))
	if err != nil {
		panic(err)
	}
	// Get the model's db
